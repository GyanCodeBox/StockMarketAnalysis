...
function StockChart({ symbol, quote, ohlcData, indicators }) {
  ...
  useEffect(() => {
    ...
    if (chartData.length >= 20 && indicators?.sma_20) {
      // Calculate 20-day SMA for each point
      const sma20Data = []
      for (let i = 19; i < chartData.length; i++) {
        const periodData = chartData.slice(i - 19, i + 1)
    ...
    if (chartData.length >= 50 && indicators?.sma_50) {
      // Calculate 50-day SMA for each point
      const sma50Data = []
      for (let i = 49; i < chartData.length; i++) {
        const periodData = chartData.slice(i - 49, i + 1)
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${
                indicators.price_trend === 'bullish' ? 'text-green-600' :
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${
                indicators.volume_analysis?.volume_trend === 'increasing' ? 'text-green-600' :
